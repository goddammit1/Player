...
3. Результаты кэшируются в RAM и в SQLite, поэтому повторные поиски не
   ...
   стороне Genius. Refresh запускается при очередном проигрывании трека
   (см. `PlayerService._fetchAndApplyArtwork`). Если нового URL нет или
   сеть недоступна, устаревший URL используется как запасной, пока TTL
   не обновится.
